...
class PersonaData
{
	...
	public function add()
	{
		$sql = "insert into persona (tipo_documento,documento,nombre,giro,direccion,tipo,fecha_creada,razon_social,celular,ocupacion,medio_transporte,destino,motivo) ";
		$sql .= "value ($this->tipo_documento,\"$this->documento\",\"$this->nombre\",\"$this->giro\",\"$this->direccion\",1,$this->fecha_creada,\"$this->razon_social\",\"$this->celular\",\"$this->ocupacion\",\"$this->medio_transporte\",\"$this->destino\",\"$this->motivo\")";
		return Executor::doit($sql);
	}
